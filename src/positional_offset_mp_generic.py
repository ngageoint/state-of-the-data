"""-----------------------------------------------------------------------------
Name: positional_offset_mp_generic.py
...
def main_task(source_features, comparative_features, source_grid, output_grid, output_features, distance, sample_size):
    ...
    # # add output fields
    add_fields = [['OFFSET_METERS', 'DOUBLE'],['OSM_MATCH', 'TEXT']]
    ...
    # update features with results
    update_with_results(output_grid, ['MEAN_OFFSET_METERS', 'PO_SCORE'], results)
...
def main(*argv):
    try:
        #  Inputs
        #
        comp_features = argv[0]
        baseline_features = argv[1]
        polygon_grid = argv[2]
        sample_size = int(argv[3])
        output_features = argv[4]
        output_grid = argv[5]
        ...
        arcpy.AddMessage(pythonExe)
        multiprocessing.set_executable(pythonExe)
        multiprocessing.freeze_support = True
        main_task(comp_features, baseline_features,
