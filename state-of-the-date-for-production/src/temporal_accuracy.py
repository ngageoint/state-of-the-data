...
class TemporalAccuracy(object):
    ...
    def __init__(self, change_detection_features, currency_features,
        ...
        #print("Years: "+''.join(self.year_list))
    ...
    def get_temporal_accuracy(self):
        ...
        #arcpy.AddMessage('Scoring the features')
            with da.UpdateCursor(self.temporal_accuracy_features,
                ...
                for row in cursor:
                    for x in range(0,self.num_years):
                        # or row[x] == 'None'
                        if row[x] == None:
                            row[x] = 0
                    ...
                    #    row[self.num_years+2] = None
                    #else:
                    if self.as_int(row[self.num_years]) >= self.years[self.num_years-1]:
                        row[self.num_years+1] = 6
                        row[self.num_years+2] = 0
                    elif self.as_int(row[self.num_years]) < self.years[0]:
                        row[self.num_years + 1], row[self.num_years + 2] = self.get_score(
                            row[0:self.num_years])
                    elif self.as_int(row[self.num_years]) == 1111 or row[self.num_years] == 'None':
                        row[self.num_years + 1], row[self.num_years + 2] = self.get_score(row[x:self.num_years])
                    #originally 2012, should be indexed at 12?
                    else:
                        try:
                            for x in range(0,self.num_years):
                                if (self.as_int(row[self.num_years]) >= self.years[x] and self.as_int(row[self.num_years]) < self.years[x+1]):
                                    row[self.num_years+1], row[self.num_years+2] = self.get_score(row[x:self.num_years])
                        except:
